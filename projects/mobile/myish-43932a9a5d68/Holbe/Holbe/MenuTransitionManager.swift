...
class MenuTransitionManager: NSObject, UIViewControllerAnimatedTransitioning, UIViewControllerTransitioningDelegate {
    ...
    func animateTransition(transitionContext: UIViewControllerContextTransitioning) {
        ...
        let moveRight = CGAffineTransformMakeTranslation(container.frame.width-50, 0)
        let resize = CGAffineTransformMakeScale(0.8, 0.8)
        ...
        UIView.animateWithDuration(duration, delay: 0.0, usingSpringWithDamping: 0.8, initialSpringVelocity: 0.8, options: [], animations: {
            ...
            if self.isPresenting {
                self.snapshot?.transform = CGAffineTransformConcat(moveRight, resize)
