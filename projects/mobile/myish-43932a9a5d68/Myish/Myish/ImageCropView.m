...
static CGFloat const DEFAULT_MASK_ALPHA = 0.85;
static bool const square = YES;
float IMAGE_MIN_HEIGHT = 800;
float IMAGE_MIN_WIDTH = 800;
...
//        IMAGE_MIN_WIDTH = [UIScreen mainScreen].bounds.size.width;
//        IMAGE_MIN_HEIGHT = IMAGE_MIN_WIDTH;
        ...
        self.cropView  = [[ImageCropView alloc] initWithFrame:view blurOn:YES];
...
- (void) viewWillAppear:(BOOL)animated {
    ...
    CGRect CropArea = CGRectMake(0 , (screenBounds.size.height/2)-(screenBounds.size.width/2), screenBounds.size.width ,screenBounds.size.width);
    self.cropArea = CropArea;
    UIView* cropAreaView = ([[UIView alloc] initWithFrame:CropArea]);
    cropAreaView.opaque = NO;
    cropAreaView.backgroundColor = [UIColor clearColor];
    
    cropAreaView.layer.borderColor = [UIColor redColor].CGColor;
    ...
    [self.view bringSubviewToFront:cropAreaView];
    
    //
//
    
     NSLog(@"imageview height=%f", self.cropView.cropImageView.frame.size.height);
//
    CGFloat originalImageHeight = self.cropView.cropImageView.frame.size.height;
    CGFloat originalImageWidth = self.cropView.cropImageView.frame.size.width;
    
    CGFloat minZoomScale = 1;
        if (self.cropView.cropImageView.frame.size.height < self.cropArea.size.height){
            minZoomScale = self.cropArea.size.height/self.cropView.cropImageView.frame.size.height;
        }
        if(self.cropView.cropImageView.frame.size.width < self.cropArea.size.width){
            minZoomScale = self.cropArea.size.width/self.cropView.cropImageView.frame.size.width;
        }
    
//    self.scrollview.minimumZoomScale = minZoomScale;
//    [self.scrollview setZoomScale:minZoomScale];
//
//    CGFloat newContentOffsetX = -(self.cropView.cropImageView.frame.size.width-self.cropArea.size.width)/2;
//    CGFloat newContentOffsetY = (self.cropArea.size.height-originalImageHeight)/2;
//    
//    CGPoint newY = [self.scrollview convertPoint:CropArea.origin fromView:self.view];
//   
//     NSLog(@"newY = %@" ,NSStringFromCGPoint(newY));
//    
//     NSLog(@"imageview height new =%f", self.cropView.cropImageView.frame.size.height);
//    
//    NSLog(@"crop area height=%f", self.cropArea.size.height);
//
//    NSLog(@"crop area origin=%f", self.cropArea.origin.y);
//    
//    
//    self.scrollview.contentOffset = CGPointMake(self.scrollview.contentOffset.x , (newY.y)*self.scrollview.zoomScale);
//    
//   
//    NSLog(@"content offset = %@" ,NSStringFromCGPoint(self.scrollview.contentOffset));
//    
...
//    CGSize zoomSize;
//    zoomSize.width = self.scrollview.bounds.size.width / minZoomScale;
//    zoomSize.height = self.scrollview.bounds.size.height / minZoomScale;
//    
//    //offset the zoom rect so the actual zoom point is in the middle of the rectangle
//    CGRect zoomRect;
//    zoomRect.origin.x = CropArea.size.width/2 - zoomSize.width / 2.0f;
//    zoomRect.origin.y = CropArea.size.height/2 - zoomSize.height / 2.0f;
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    NSLog(@"content offset after scrolling = %@" ,NSStringFromCGPoint(self.scrollview.contentOffset));
}

- (UIView *)viewForZoomingInScrollView:(UIScrollView *)scrollView
{
    
    return self.cropView.cropImageView;
}
...
- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale{
    ...
    scrollView.contentSize = CGSizeMake(self.view.frame.size.width*scale, self.view.frame.size.height*scale);
    
     NSLog(@"content offset after zooming = %@" ,NSStringFromCGPoint(self.scrollview.contentOffset));
}
...
//
//            CGRect visibleRect = CGRectMake(self.scrollview.contentOffset.x * scale, self.scrollview.contentOffset.y*scale, self.scrollview.bounds.size.width*scale, self.scrollview.bounds.size.height*scale);
            //CGFloat scale = 1/self.scrollview.zoomScale;//self.image.scale;
//            
//            CGRect rect = self.cropArea;
//            
//            CGRect cropRect;
//            
//           
//          cropRect = CGRectMake(self.scrollview.contentOffset.x,
...
//            CGImageRelease(imageRef);
            ...
            CGRect cropRect = CGRectMake(rect.origin.x+(Scale*2), (rect.origin.y*Scale)+(Scale*2), (rect.size.width*Scale)-(Scale*4), (rect.size.height*Scale)-(Scale*4));
            CGImageRef imageRef = CGImageCreateWithImageInRect([snapshot CGImage], cropRect) ;
...
//                                      orientation:self.image.imageOrientation];
            ...
            if (self.cropView.imageview.frame.size.height < rect.size.height)   {
