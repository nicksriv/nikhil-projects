...
public class DraggableCardView: UIView {
    ...
    private var originalLocation: CGPoint = CGPoint(x: 0.0, y: 50.0)
    ...
    override public var frame: CGRect {
        didSet {
            actionMargin = (frame.size.width / 2.0)-30
        }
    }
    ...
    private func UpAction() {
        ...
        self.overlayView?.overlayState = OverlayMode.Right
        self.overlayView?.alpha = 1.0
        self.delegate?.cardSwippedInDirection(self, direction: SwipeResultDirection.Up)
        UIView.animateWithDuration(cardSwipeActionAnimationDuration,
            delay: 0.0,
            options: .CurveLinear,
            animations: {
