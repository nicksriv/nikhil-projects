...
var generateJson = function generateJson(props) {
  ...
  var pageId = 1;
  var sectionId = 0;
  var isSection = false; // let pages = new Array();
  // let pageCount = 0;
  ...
  for (var index = 0; index < elements.length; index++) {
    switch (elements[index].element) {
      ...
      //         break;
      case 'Page_Break':
        pageId++; // let pageBreak = elements[index];
      ...
      case 'Section_Header':
        sectionId++;
        isSection = true; // if(currentSection != null){
        //     currentPage.sections.push(currentSection);
        //     sectionCount++;
        // }
  ...
  try {
    ...
    props.getJSON(root); // store.dispatch('updateOrder', []);
