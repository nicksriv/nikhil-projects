...
import { addJobService } from 'app/redux/JobManagement/JobManagementService'
...
const jobBillingTimingInitialState = {
    number: '',
    type: '',
    hourRequired: '',
    hourRequiredPer: '',
...
const AddJob = () => {
    ...
    const [deliverableChipText, setDeliverableChipText] = useState('')
    ...
    const { activeStep, formBtnStates, weekDays, skillsList, countries } =
   ...
   const clientId = localStorage.getItem('selectedClientLogo');
    ...
    const handleInputChange = (key, name, value, e) => {
        if (key === 'jobSummary') {
          setJobSummary((prev) => ({
        ...
        } else if (key === 'jobSetting') {            
            if (name === 'visibilityType') {
                          setIsVisibilityValueEnable(value === 'WORLDWIDE');
            setJobSetting((prev) => ({
              ...prev,
              visibilityValue: value === 'WORLDWIDE' ? [] : prev.visibilityValue,
            ...
            //validation for the lat and lng fields
    ...
    const handleSaveDraft = async () => {
        try {
            ...
            console.log(JSON.stringify(payload), 'payload for on add job')
            const res = await addJobService(payload)
            if (!res.error) {
    ...
    const handleResetForm = () => {
        ...
        setJobSetting(jobSettingInitialState)
        setJobBillingTiming(jobBillingTimingInitialState)
        setDeliverableChip([])
        setHighlightsChip([])
        setJobDescription('')
    }
                        ...
                        <Grid item>
                            {activeStep === 0 && (
                                <JobSummary
                                    ...
                                    deliverableChipText={deliverableChipText}
                                    deliverableChip={deliverableChip}
                                    highlightsChip={highlightsChip}
                                    handleDeliverables={addDeliverables}
                                    removeDeliverables={removeDeliverables}
                                    handleHighlights={addHighlights}
