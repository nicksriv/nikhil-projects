...
import { Button, ClickAwayListener, FormControl, InputBase, MenuItem, Select } from '@material-ui/core';
...
function ReportList(props) {
    ...
    const { totalRecords } = useSelector((state) => state.report);
    ...
    const handleEdit = (data) => {
        if (data && data.id) {
            history.push(`/report/edit?id=${data.id}`);
        }
    }
    const handleSort = (columnName) => {
        ...
        dispatch({
            type: "setReportSortDirectionAction",
    ...
    const handleView = (data) => {
        if (data && data.id) {
            history.push(`/report/view?id=${data.id}`);
        }
    }
    async function confirmDelete() {
        try {
            await dispatch({ type: 'deleteReportByIdAction', payload: { reportId: selectedReportId } });
            dispatch({
                type: "getAllReportsDataAction",
        ...
        <div>
            ...
            <div className={classes.paginationContainer}>
                <div className="flex items-end justify-centers">
                    <Paginate page={page} paginate={paginate} pageSize={pageSize} listSize={totalRecords} />
