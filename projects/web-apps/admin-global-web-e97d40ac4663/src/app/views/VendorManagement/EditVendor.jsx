...
import { updateVendorService } from 'app/redux/VendorManagement/VendorManagementService'
...
const EditVendor = () => {
    ...
    const { vendorDetails, loading } = useSelector(
    ...
    const handlePrefilledData = () => {
        if (vendorDetails) {
            ...
            const { name, email, mobile, designation } = spocDetail

            let preFilledSkills = []
            const data = skillCategory?.forEach((item) => {
                preFilledSkills = [...preFilledSkills, ...item.skills]
            })
            setBasicDetails({
                ...basicDetails,
                vendorName: vendorName,
                workHighlights: workDetails,
                companyIncorporatedAt: companyIncorporatedAt,
    ...
    const handleOnSave = async () => {
        try {
            const payload = generatePayload()
            const res = await updateVendorService({ id, payload })
            if (!res.error) {
