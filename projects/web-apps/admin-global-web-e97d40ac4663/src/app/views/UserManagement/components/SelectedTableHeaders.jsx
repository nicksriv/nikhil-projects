...
export default function SelectedTableHeaders({
    heading,
    pageSize,
    tableData,
    filterDetails,
    ...
    const handleEdit = (selectedClient) => {
        if (selectedClient && selectedClient.userId) {
            history.push(
                `/user/edit?clientId=${clientIdForUsers}&id=${selectedClient.userId}`
            )
        }
    }
    ...
    function handleInfo(data) {
        ...
        if (status === statuses[0]) {
            dispatch({
                type: 'getUserCredentialDetailsAction',
                payload: { userId },
            })
    ...
    const handleInfoBtnClick = (status) => {
        ...
        if (status === statuses[1] || status === statuses[2]) {
            history.push(
                `/user/edit?clientId=${clientIdForUsers}&id=${selectedUserId}`
            )
        }
    }
    ...
    const handleSort = (columnName) => {
        ...
        let converToUpperCase = updatedColumnName
            .toUpperCase()
            .split(' ')
            .join('_')
