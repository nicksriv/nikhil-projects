...
export default function DisputesList({
    ...
    const handleView = (selectedDisputeId) => {
        if (selectedDisputeId && selectedDisputeId.id) {
            const id = selectedDisputeId.id
            history.push(`/dispute/disputeDetails/${id}`)
        }
    }

    const handleInfo = (selectedDisputeId) => {
        if (selectedDisputeId && selectedDisputeId.id) {
            const id = selectedDisputeId.id
            history.push(`/dispute/disputeDetails/${id}`)
        }
    ...
    return (
        <div style={{ padding: '5px' }}>
            ...
            <DataTable
                ...
                setIsOpen={setIsOpen}
                isOpen={isOpen}
                filterDetails={() => { console.log("filter") }}
            />
