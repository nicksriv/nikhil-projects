...
function SiteList({
    ...
    const { siteList, siteFilterDetails } = useSelector((state) => state.sites);
    ...
    const handleView = (selectedSite) => {
        if (selectedSite
            && selectedSite.id) {
            const id = selectedSite.id;
            history.push(`/site/view?id=${id}`);
        }
    }
    const handleEdit = (selectedSite) => {
        if (selectedSite && selectedSite.id) {
            const id = selectedSite.id
            history.push(`/site/edit?id=${id}`);
        }
    }
    const handleDelete = () => {
    ...
    const handleSort = (columnName) => {
        ...
        dispatch({
            type: "setSiteSortDirectionAction",
            direction: sortDirection,
            sortBy: columnToSort
        })
        dispatch({ type: "getAllSiteDetailsAction", payload: { clientId: clientIdForUsers, pageNumber: pageNumber, size: pageSize, sortBy: converToUpperCase, sortOrder: sortDirection, filter: siteFilterDetails } });
    }
    useEffect(() => {
        ...
        <div>
            ...
            >
                <ClickAwayListener onClickAway={() => setIsOpen(false)}>
                    {/* Keep your code here */}
                    <div className={`${classes.popOverMenu} py-3`}>
                                ...
                                >
                                    {el.icon ? null : (
                                        <Button
                                            ...
                                            className={`w-full mt-2 pl-3 pr-3 ml-5 mr-5 justify-start ${
                                                checkedValues.includes(el.key)
            ...
            <div className={classes.paginationContainer}>
                <div className="flex items-end justify-centers">
                    <Paginate page={page} paginate={paginate} pageSize={pageSize} listSize={siteList.size} />
            ...
            />
            <DetailsDailogue
                description="This person delivers excellent customer service that ensures ongoing sales and high levels of customer satisfaction. You will Work with customers to determine their needs and then recommend the right product to solve the customer’s problem. Also develop and cultivate strong buying relationships with customers."
                hasBtn={false}
                open={showInfoPopup}
