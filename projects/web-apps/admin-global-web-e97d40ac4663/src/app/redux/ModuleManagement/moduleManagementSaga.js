...
function* getAllFormsBySubModuleId(payload) {
    try {
        const { moduleId, submoduleId, filters, from, to, employeeId, name, empRole, page, size, sortBy, sortOrder, isFromApplyFilter, mappedBy
        } = payload.payload;
        let request = {
            size: size,
            filters: filters,
...
function* submitScreenFormDataSmId(payload) {
    try {
        const { moduleId, subModuleId, data, workflowId } = payload.payload;
        const response = yield call(submitScreenFormDataForSubModuleService, moduleId, subModuleId, data, workflowId);
        yield put(SNACKBAR_SUCCESS(response.message));
    } catch (error) {
        yield put(SNACKBAR_ERROR(error.message));
    }
