...
export default class TakePhotoOptions extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
    ...
    return (
      <>
        <Grid container spacing={2} className="padding-top-10">
          <Grid>
            <Grid item>
              <h6 style={text} className='pl-2'>Camera Facing: </h6>
            </Grid>
            <Grid item>
            <FormControl component="fieldset">
              <RadioGroup row aria-label="material" name="material" value={this.props.element.customOptions.cameraFacingOptions} onChange={(event) => this.props.changeCameraOptions('cameraFacingOptions', event)}>
                <FormControlLabel value="front" style={this.props.element.customOptions.cameraFacingOptions === 'front' ? checked : unchecked } control={<Radio color="primary" icon={<CircleUnchecked className="checkboxSize"/>} checkedIcon={<CircleCheckedFilled  className="checkboxSize"/>} />} label="Front" labelPlacement='start' />
                <FormControlLabel value="rear" style={this.props.element.customOptions.cameraFacingOptions === 'rear' ? checked : unchecked } control={<Radio color="primary" icon={<CircleUnchecked className="checkboxSize"/>} checkedIcon={<CircleCheckedFilled className="checkboxSize" />} />} label="Rear" labelPlacement='start' />
                <FormControlLabel value="both" style={this.props.element.customOptions.cameraFacingOptions === 'both' ? checked : unchecked } control={<Radio color="primary" icon={<CircleUnchecked className="checkboxSize"/>} checkedIcon={<CircleCheckedFilled className="checkboxSize" />} />} label="Both" labelPlacement='start' />
              </RadioGroup>
            </FormControl>
            </Grid>
          </Grid>
          <Grid item xs={12} className='mt-3'>
            <h6 style={text}>Photo Options: </h6>
          </Grid>
          {
            options.map((option) => {
              return (
                <Grid item xs={12} sm={6} >
                  <FormControlLabel
                   style={this.props.element.customOptions[option.name]? checked1 : unchecked1}
                    label={option.label}
                    control={
                    <Checkbox style={{opacity:0,width:"5px"}} className="mr-1 ml-2" checked={this.props.element.customOptions[option.name]} 
          ...
          {/* <Grid item xs={12}>
            <Typography variant="h6" component="h6">Custom Options</Typography>
          </Grid>
          <Grid item xs={12} sm={12} className="element-options-border-grid">
            <FormControlLabel
              control={<Checkbox checked={this.props.element.customOptions.tapedrop} onChange={(event) => this.props.editElementCustomOptionsProp(event.target.name, 'checked', event)} name="tapedrop" color="primary" />}
              label="Tapedrop"
            />
            <FormControlLabel
              control={<Checkbox checked={this.props.element.customOptions.isDistToBuilding} onChange={(event) => this.props.editElementCustomOptionsProp(event.target.name, 'checked', event)} name="isDistToBuilding" color="primary" />}
              label="Distance to Building"
            />
            <FormControlLabel
              control={<Checkbox checked={this.props.element.customOptions.isTargetAzimuthAngle} onChange={(event) => this.props.editElementCustomOptionsProp(event.target.name, 'checked', event)} name="isTargetAzimuthAngle" color="primary" />}
              label="TargetAzimuth Angle"
            />
          </Grid> */}
          {
            this.props.element.customOptions["isShowHelp"] &&
            <Grid item xs={12} className="element-options-border-grid">
              <TextField
                ...
                onChange={(event) => this.props.setDefaultOptions("helpText", event)}
                inputProps={{ type: 'text' }} />
            </Grid>
          }
          {
            this.props.element.customOptions["isDistToBuilding"] &&
            <Grid item xs={12} className="element-options-border-grid">
              <TextField
                size="small"
                variant="outlined"
                ...
                onChange={(event) => this.props.setDefaultOptions("distanceToBuildingRadius", event)}
                inputProps={{ type: 'text' }} />
            </Grid>
          }
          {/* <Grid item xs={12} className="element-options-border-grid">
            <TextField
              ...
              style={{ width: '50%' }}
              label="Sample S3 Uri"
              value={this.props.element.customOptions.sampleS3Uri}
              onChange={(event) => this.props.setDefaultOptions("sampleS3Uri", event)}
              inputProps={{ type: 'text' }} />
          </Grid> */}
        </Grid>
        <Grid container spacing={2} className="padding-top-10">
